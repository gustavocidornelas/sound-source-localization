...
if __name__ == '__main__':
    ...
    LCR = np.genfromtxt('/Users/gustavocidornelas/Desktop/sound-source/decaying_sinusoid_0.99_gamma_0.999_Az_90_freq_'
                        '80.0.csv', delimiter=',')
    LCR_L = LCR[501:, 1]
    LCR_R = LCR[501:, 2]

    # separating the samples of interest
    #LCR_L = LCR_L[242000: 242000 + 50] # 242000
    #LCR_R = LCR_R[242000:242000 + 50]

    # reading the coefficients (already transformed to the canonical basis)
    coeff_L = np.genfromtxt('transformed_poly_fit_result_LEFT.csv', skip_header=False, delimiter=',')
    coeff_L = coeff_L[242000, :]
    coeff_R = np.genfromtxt('transformed_poly_fit_result_RIGHT.csv', skip_header=False, delimiter=',')
