...
def local_poly_fit(signal):
    ...
    for index in range(2):
        ...
        for k0 in range(b + 1, L + a - 1):
            ...
            xi_k_f = np.dot(A_poly_inv, xi_k_f - A_a_s * hrir[k0 - 1 + a] + s_poly * hrir[k0 - 1])

            # backward pass
            k_k_b = k_k_b - np.power(hrir[k0 - 1], 2) + np.power(hrir[k0 + b], 2)
            xi_k_b = np.dot(A_poly_inv, xi_k_b - s_poly * hrir[k0 - 1] + A_b_1_s * hrir[k0 + b])
...
def delay_estimation():
    ...
    for az in azimuth:
        ...
        coeff_left = np.genfromtxt('/Users/gustavocidornelas/Desktop/sound-source/Other methods/coeff_HRIR_left_Az_' +
                                   str(az) + '.csv', delimiter=',', skip_header=False)
        coeff_right = np.genfromtxt('/Users/gustavocidornelas/Desktop/sound-source/Other methods/coeff_HRIR_right_Az_' +
                                    str(az) + '.csv', delimiter=',', skip_header=False)
        ...
        for k in range(coeff_left.shape[0]):
            ...
            if abs(coeff_left_k[1]) < 4e-4 and 2 * coeff_left_k[2] < 0 and left_peak == 0:
                left_peak = k
                print('Left!')
            if abs(coeff_right_k[1]) < 4e-4 and 2 * coeff_right_k[2] < 0 and right_peak == 0:
                right_peak = k
                print('Right!')
