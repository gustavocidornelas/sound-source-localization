...
if __name__ == '__main__':
    ...
    alpha = np.genfromtxt('/Users/gustavocidornelas/Desktop/sound-source/Other methods/signal_left.csv',
                          delimiter=',', skip_header=False)
    # beta is the set of coefficients for the right
    beta = np.genfromtxt('/Users/gustavocidornelas/Desktop/sound-source/Other methods/signal_right.csv',
                         delimiter=',', skip_header=False)
    ...
    for k in range(a, alpha.shape[0] - b):
        ...
        # observation
        observation_coef = A @ np.kron(alpha_k, alpha_k) - B @ np.kron(alpha_k, beta_k) + C @ np.kron(beta_k, beta_k)
    ...
    audio = np.genfromtxt('/Users/gustavocidornelas/Desktop/sound-source/ISTS Signal/Speech signals/Speech_Az_30.csv',
                          delimiter=',')
    audio_onset = audio[238000:238600, :]

    fig, axs = plt.subplots(2)
    axs[0].set_title('Continuous delay estimation for the azimuth 30 degrees')
    axs[0].plot(range(audio_onset.shape[0]), audio_onset[:, 0], label='Left', linewidth=2)
    axs[0].plot(range(audio_onset.shape[0]), audio_onset[:, 1], label='Right', linewidth=2)
    #axs[0].set_xlim(0, 350)
    axs[1].plot(range(len(ITD)), ITD, label='Continuous estimate', linewidth=2)
    axs[1].plot(range(smooth_delay.shape[0]), smooth_delay, label='Moving average', linewidth=2)
    axs[1].plot(range(smooth_delay.shape[0]), -0.221 * np.ones(smooth_delay.shape[0]), 'k--', label='Theoretical delay')
    #axs[1].set_ylim(0, 1)
