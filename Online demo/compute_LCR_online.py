...
class ComputeLCROnline:
    ...
    """
    def __init__(self, onset_model, window_model, frequencies, n_freq, onset_decays, window_decay):
        ...
        assert len(self.frequencies) == len(self.onset_decays), 'Number of frequencies and decays do not match.'
        assert len(self.frequencies) == self.n_freq, 'Length of frequency list does not match the number of ' \
        ...
        if self.onset_model == 'decaying_sinusoid':
            ...
            self.P = np.kron(np.eye(self.n_freq, dtype=int), np.hstack((np.eye(2), np.zeros((2, 6)))))
    ...
    def compute_lcr(self, y, chi_k, zeta_k, s_k, w_k, xi_k, k_k):
        ...
        chi_k = np.dot(self.window.A, chi_k) + self.window.s * (np.power(y_k[0, 0], 2) + np.power(y_k[1, 0], 2))
        temp_chi_k_L = np.dot(self.window.A, chi_k) + self.window.s * np.power(y_k[0, 0], 2)  # just for the left channel
        temp_chi_k_R = np.dot(self.window.A, chi_k) + self.window.s * np.power(y_k[1, 0], 2)  # just for the right channel
        zeta_k = np.dot(self.A_model_window, zeta_k) + np.dot(self.s_model_window, np.transpose(y_k))
        s_k = np.linalg.multi_dot([self.A_model_window, s_k, np.transpose(self.model.A)]) + np.dot(self.s_model_window,
                                                                                         np.transpose(self.model.s))
        ...
        for row in range(self.n_states):
            for col_xi in range(M):
                ...
                xi_k[row, col_xi] = np.trace(np.dot(np.kron(mask_xi, self.window.C), zeta_k))
        ...
        if self.onset_model == 'decaying_sinusoid':
            ...
            min_J_L = temp_k_k_L - np.linalg.multi_dot([np.transpose(xi_k[:, 0]), np.linalg.pinv(w_k), xi_k[:, 0]])
            min_J_R = temp_k_k_R - np.linalg.multi_dot([np.transpose(xi_k[:, 1]), np.linalg.pinv(w_k), xi_k[:, 1]])

            min_J = k_k - np.trace(np.linalg.multi_dot([np.transpose(xi_k), np.linalg.pinv(w_k), xi_k]))

        elif self.onset_model == 'gammatone':
            # determining the output matrix for the onset model
            PWPt_inv = np.linalg.pinv(np.linalg.multi_dot([self.P, w_k, np.transpose(self.P)]))
            C_onset = np.dot(np.transpose(np.linalg.multi_dot([PWPt_inv, self.P, xi_k])), self.P)

            min_J_L = temp_k_k_L - np.linalg.multi_dot([np.transpose(xi_k[:, 0]), np.linalg.pinv(w_k), xi_k[:, 0]])
            min_J_R = temp_k_k_R - np.linalg.multi_dot([np.transpose(xi_k[:, 1]), np.linalg.pinv(w_k), xi_k[:, 1]])

            # calculating the minimum cost
            min_J = k_k - np.trace(np.linalg.multi_dot([np.transpose(xi_k), np.transpose(self.P), PWPt_inv, self.P,
                                                        xi_k]))
