...
if __name__ == '__main__':
    ...
    window_model = 'gamma'
    burn_in = 30000  # number of samples of the the LCR that are thrown away at the beginning
    ...
    LCR_mod = ComputeLCROnline(onset_model, window_model, frequencies, n_freq, onset_decays, window_decay)
    ...
    for k in range(audio.shape[0]):
        ...
        chi_k, zeta_k, s_k, w_k, xi_k, k_k, LCR[k, :] = LCR_mod.compute_lcr(y, chi_k, zeta_k, s_k, w_k, xi_k, k_k)
        ...
        if k == burn_in + 201:
            ...
            k_k_f_L, k_k_b_L, xi_k_f_L, xi_k_b_L, \
            W_k_f_L, W_k_b_L, C_opt_L[k, :], LCR_window_L = poly_mod.initialize_messages(LCR_window_L,k_k_f_L, k_k_b_L,
                                                                                         xi_k_f_L, xi_k_b_L, W_k_f_L,
                                                                                         W_k_b_L)
            k_k_f_R, k_k_b_R, xi_k_f_R, xi_k_b_R, \
            W_k_f_R, W_k_b_R, C_opt_R[k, :], LCR_window_R = poly_mod.initialize_messages(LCR_window_R, k_k_f_R, k_k_b_R,
                                                                                         xi_k_f_R, xi_k_b_R, W_k_f_R,
                                                                                         W_k_b_R)
        elif k > burn_in + 201:
            # moving the window forward
            LCR_new_L = LCR[k, 0]
            k_k_f_L, k_k_b_L, xi_k_f_L, xi_k_b_L,\
            W_k_f_L, W_k_b_L, C_opt_L[k, :], LCR_window_L = poly_mod.fit_polynomial(LCR_window_L, LCR_new_L, k_k_f_L,
                                                                                    k_k_b_L, xi_k_f_L, xi_k_b_L,
                                                                                    W_k_f_L, W_k_b_L)
            LCR_new_R = LCR[k, 1]
            k_k_f_R, k_k_b_R, xi_k_f_R, xi_k_b_R, \
            W_k_f_R, W_k_b_R, C_opt_R[k, :], LCR_window_R = poly_mod.fit_polynomial(LCR_window_R, LCR_new_R, k_k_f_R,
                                                                                    k_k_b_R, xi_k_f_R, xi_k_b_R,
                                                                                    W_k_f_R, W_k_b_R)
