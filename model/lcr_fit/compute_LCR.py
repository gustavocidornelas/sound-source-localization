...
class ComputeLCR:
    ...
    def compute_lcr(self):
        ...
        assert (
            len(f) == n_freq
        ), "Length of frequency list does not match the number of frequencies specified."
        ...
        if onset_model == "decaying_sinusoid":
            ...
            P = np.kron(
                np.eye(n_freq, dtype=int), np.hstack((np.eye(2), np.zeros((2, 6))))
            )
        ...
        for k in range(L):
            ...
            zeta_k = np.dot(A_model_window, zeta_k) + np.dot(
                s_model_window, np.transpose(y_k)
            )
            s_k = np.linalg.multi_dot(
                [A_model_window, s_k, np.transpose(model.A)]
            ) + np.dot(s_model_window, np.transpose(model.s))
            ...
            for row in range(n_states):
                for col_xi in range(M):
                    ...
                    xi_k[row, col_xi] = np.trace(
                        np.dot(np.kron(mask_xi, window.C), zeta_k)
                    )
            ...
            if onset_model == "decaying_sinusoid":
                ...
                PWPt_inv = np.linalg.pinv(
                    np.linalg.multi_dot([P, w_k, np.transpose(P)])
                )
                C_onset = np.dot(
                    np.transpose(np.linalg.multi_dot([PWPt_inv, P, xi_k])), P
                )
                ...
                min_J[k] = k_k - np.trace(
                    np.linalg.multi_dot(
                        [np.transpose(xi_k), np.transpose(P), PWPt_inv, P, xi_k]
                    )
                )
