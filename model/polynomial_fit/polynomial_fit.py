...
class FitPoly:
    ...
    def fit_polynomial(self, index):
        ...
        M = LCR.shape[
            1
        ]  # number of channels (treating the LCR as a multi-channel signal)
        ...
        for k0 in range(b + 1, L + a - 1):
            ...
            xi_k_f = np.dot(
                A_poly_inv, xi_k_f - A_a_s * LCR_L[k0 - 1 + a] + s_poly * LCR_L[k0 - 1]
            )

            # backward pass
            k_k_b = k_k_b - np.power(LCR_L[k0 - 1], 2) + np.power(LCR_L[k0 + b], 2)
            xi_k_b = np.dot(
                A_poly_inv, xi_k_b - s_poly * LCR_L[k0 - 1] + A_b_1_s * LCR_L[k0 + b]
            )
