...
def display_plots(azimuth, signal_id, fs=44100.0, burn_in=30000):
    ...
    azimuth_data_path = (
        "/Users/gustavocidornelas/Desktop/sound-source/Dataset from GRID/Az_"
        + str(azimuth)
    )
    ...
    audio_signal = np.load(
        azimuth_data_path + "/" + signal_id + "_Az_" + str(azimuth) + ".npy"
    )
    audio_signal = audio_signal[burn_in:, :]
    all_lcr_files = [
        file for file in os.listdir(lcr_path) if file.startswith(signal_id)
    ]
    all_delay_files = [
        file for file in os.listdir(delay_path) if file.startswith(signal_id)
    ]
    all_coeff_files = [
        file for file in os.listdir(coeff_path) if file.startswith(signal_id)
    ]
    ...
    fig, axs = plt.subplots(len(all_lcr_files) - 1)
    ...
    for i, lcr_file in enumerate(all_lcr_files):
        ...
        if freq in ["[160.0]", "[200.0]", "[240.0]"]:
            # plotting
            axs[i].plot(np.asarray(range(lcr.shape[0])), lcr[:, 1], "b")
            axs[i].plot(np.asarray(range(lcr.shape[0])), lcr[:, 2], "r")
            axs[i].set_title(freq + " Hz")
