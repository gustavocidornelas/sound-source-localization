...
def convolve_hrir(azimuth, speech_signal):
    ...
    hrir = np.genfromtxt(
        "/Users/gustavocidornelas/Desktop/sound-source/ISTS Signal/HRIR/HRIR_Az_"
        + str(azimuth)
        + ".csv",
        delimiter=",",
    )
    ...
    for az in azimuths:
        ...
        for region_number in range(8):
            if gender_mask[region_number]:
                # pick female speaker
                all_speakers = os.listdir(
                    "/Users/gustavocidornelas/Desktop/sound-source/TIMIT/TRAIN/DR"
                    + str(region_number + 1)
                )
                all_female = [person for person in all_speakers if person[0] == "F"]
                female_speaker = all_female[
                    np.random.randint(low=0, high=len(all_female), size=1)[0]
                ]
                ...
                all_signals = [
                    ...
                    for file in os.listdir(
                        "/Users/gustavocidornelas/Desktop/sound-source/TIMIT/TRAIN/"
                        "DR" + str(region_number + 1) + "/" + female_speaker
                    )
                    if file.endswith("_new.wav")
                ]
                female_signal = all_signals[
                    np.random.randint(low=0, high=len(all_signals), size=1)[0]
                ]

                # reading the chosen signal
                fs, speech_signal = wavfile.read(
                    "/Users/gustavocidornelas/Desktop/sound-source/TIMIT/TRAIN/"
                    "DR"
                    + str(region_number + 1)
                    + "/"
                ...
                az_info["DR" + str(region_number + 1)] = (
                    "/" + female_speaker + "/" + female_signal
                )
            else:
                # pick male speaker
                all_speakers = os.listdir(
                ...
                male_speaker = all_male[
                    np.random.randint(low=0, high=len(all_male), size=1)[0]
                ]
                ...
                # registering information about the chosen signal
                az_info["DR" + str(region_number + 1)] = (
                    "/" + male_speaker + "/" + male_signal
                )

        # saving information about the signals used for this azimuth
        f = open(
            "/Users/gustavocidornelas/Desktop/sound-source/Dataset/Az_"
            + str(az)
            + "/info.txt",
            "w",
        )
