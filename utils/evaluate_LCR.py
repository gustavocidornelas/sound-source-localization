...
if __name__ == "__main__":
    # audio data
    audio = np.genfromtxt(
        "/Users/gustavocidornelas/Desktop/sound-source/ISTS Signal/Speech signals/Speech_Az_-80.csv",
        delimiter=",",
    )
    audio = audio[1000:, :]
    ...
    LCR_1 = np.genfromtxt(
        "/Users/gustavocidornelas/Desktop/sound-source/decaying_sinusoid_[0.99]_gamma_0.999_Az_-55_freq_[80.0].csv",
        delimiter=",",
        skip_header=True,
    )
    LCR_1 = LCR_1[1000:, :]
    ...
    axs[1].plot(
        np.asarray(range(LCR_1.shape[0])) / 44100.0, LCR_1[:, 1], "b", linewidth=1.2
    )
    axs[1].plot(
        np.asarray(range(LCR_1.shape[0])) / 44100.0, LCR_1[:, 2], "r", linewidth=1.2
    )
