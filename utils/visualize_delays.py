...
if __name__ == '__main__':
    ...
    azimuths = [-80, -65, -55, -45, -40, -35, -30, -25, -20, -15, -10, -5, 0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 55, 65,
                80]
    ...
    for az in azimuths:
        ...
        delays = np.genfromtxt('/Users/gustavocidornelas/Desktop/sound-source/unique_delays_Az_' + str(az) + "_freq_[80.0].csv",
    ...
    thresh_delays = np.load('/Users/gustavocidornelas/Desktop/sound-source/Other methods/delays_threshold.npy')
    corr_delays = np.load('/Users/gustavocidornelas/Desktop/sound-source/Other methods/delays_cross_corr.npy')
    group_delays = np.genfromtxt('/Users/gustavocidornelas/Desktop/sound-source/ISTS Signal/group_delays.csv', delimiter=',')
    ...
    ax.plot(np.deg2rad(np.array(azimuths)), abs(thresh_delays), label='Threshold method (-30 dB)', linewidth=2)
    #plt.legend()
    #plt.show()
    ...
    ax.plot(np.deg2rad(np.array(azimuths)), abs(np.array(theo_delay)), label='Theoretical delay', linewidth=2)
    ax.plot(np.deg2rad(np.array(azimuths)), abs(corr_delays), label='Max cross-correlation method', linewidth=2)
    ax.plot(np.deg2rad(np.array(azimuths)), abs(group_delays*1e4), label='Group delay (multiplied by 1e4)', linewidth=2)
    plt.legend()
    plt.show()
