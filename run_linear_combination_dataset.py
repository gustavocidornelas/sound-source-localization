    ...
    for az in azimuth:
        ...
        for signal in all_signals:
            ...
            LCR_file = lcr_saving_path + "/" + signal_id + "_" + str(onset_model) + "_" + str(onset_decays) + "_" \
                       + str(window_model) + "_" + str(window_decay) + "_Az_" + str(az) + "_freq_" + str(frequencies) \
                       + ".csv"
            ...
            lcr = ComputeLCR(onset_model=onset_model, window_model=window_model, frequencies=frequencies, n_freq=n_freq,
                             ...
                             signal_id=signal_id, lcr_saving_path=lcr_saving_path)
            lcr.compute_lcr()

            # fitting the polynomial to the LCRs
            fit = FitPoly(frequencies=frequencies, LCR_file=LCR_file, azimuth=az, signal_id=signal_id,
            ...
            delay = DelayEstimation(coeff_left_file=coeff_left_file, coeff_right_file=coeff_right_file,
                                    frequencies=frequencies, azimuth=az, signal_id=signal_id,
